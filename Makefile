.PHONY: all clean clean-data clean-all init cjsh cmd build-image image run run-shell-only run-graphical run-graphical-shell-only run-g run-gs
ROOT      := $(CURDIR)
BUILD     := $(ROOT)/build
...
# guest sees the OS image as /dev/vda and the data disk as /dev/vdb without
# needing module loading.
...
$(BUILD):
	mkdir -p $(BUILD)
# Persistent data disk. Created once on first build; never rebuilt automatically
# so writes from inside the guest survive across `make image` rebuilds.
# Use `make clean-data` to wipe it deliberately.
$(DATA): | $(BUILD)
...
cmd: build-image | $(BUILD)
	...
	cid=$$(docker create $(DOCKER_IMAGE)); \
	  ...
	  docker rm $$cid >/dev/null
# image: stage rootfs and pack it into a raw ext4 disk image. Depends on
# $(DATA) so the persistent data disk gets created on first build.
image: $(BUILD)/init $(BUILD)/cjsh cmd $(DATA)
	$(MAKE) -C $(DISK) image
...
# clean: rebuild artifacts only. Preserves $(DATA) so guest writes survive.
clean:
	$(MAKE) -C $(USERLAND) clean
	$(MAKE) -C $(DISK) clean
	rm -rf $(BUILD)/init $(BUILD)/cjsh $(BUILD)/cmd $(BUILD)/disk.img
# clean-data: wipe only the persistent data disk.
clean-data:
	rm -f $(DATA)
# clean-all: nuke everything, including persistent data.
