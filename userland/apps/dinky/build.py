#!/usr/bin/env python3
"""Build dinky inside a Debian-bookworm Rust container and install the binary.

Builds with CARGO_TARGET_DIR pointing at the top-level build/dinky-target
...
SCRIPT_DIR = Path(__file__).resolve().parent              # userland/apps/dinky
...
def main() -> int:
    ...
    ap.add_argument("dest", type=Path, help="path to install the built dinky binary")
    args = ap.parse_args()
    ...
    binary = TARGET_DIR / "release" / "dinky"
