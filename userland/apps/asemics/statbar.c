...
char *getModeStr(void) {
    switch (E.mode) {
    case MODE_INSERT: return "-- INSERT -- "; break;
    case MODE_COMMAND: return "-- COMMAND -- "; break;
    case MODE_VISUAL: return "-- VISUAL -- "; break;
    case MODE_NORMAL: return "-- NORMAL -- "; break;
...
void editorDrawStatusBar(WriteBuf *wb) {
    ...
    int modelen = snprintf(mdstatus, sizeof(mdstatus), "%-10s", getModeStr());

    int statuslen = snprintf(status, sizeof(status), "%.20s - %d lines %s",
                             E.buffer.filename ? E.buffer.filename : "[No Name]", E.buffer.num_rows,
                             E.buffer.dirty ? "*" : "");

    int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d",
                        E.syntax ? E.syntax->filetype : "no ft", E.cursor.y + 1, E.buffer.num_rows);
    ...
    if (modelen + statuslen + rlen > E.viewport.width)
        rlen = E.viewport.width - modelen - statuslen;
    ...
    while (written < E.viewport.width + LINE_NUM_RESERVE - rlen) {
        writeBufAppend(wb, " ", 1);
        written++;
    }
...
void editorDrawMsgBar(WriteBuf *wb) {
    ...
    if (msglen && time(NULL) - E.ui.msg_time < 5) writeBufAppend(wb, E.ui.msg, msglen);
}
